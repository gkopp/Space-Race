...
def advance_simulation(evt): # move planets to configuration at date specified in menus
    ...
    m_day = day_menu.GetSelection() # get entered day from day menu
    if m_day == 0:                  # don't use "Day" menu label as legitimate date
        return
    day = m_day/365                 # convert to fraction of an Earth year

    m_month = month_menu.GetSelection() # get selected month from month menu
    if m_month == 0:                    # don't use "Month" menu label as legitimate date
        return
    month = m_month/12                  # convert to fraction of an Earth year

    m_year = year_menu.GetSelection()   # get selected year from year menu
    if m_year == 0:                     # don't use "Year" menu label as legitimate date
        return
